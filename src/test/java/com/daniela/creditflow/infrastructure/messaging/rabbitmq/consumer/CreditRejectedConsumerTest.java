...
class CreditRejectedConsumerTest {

    @Mock
    private NotificationService notificationService;

    @InjectMocks
    private CreditRejectedConsumer consumer;

    @Test
    void shouldNotifyRejectedCredit() {
        CreditMessage message = mock(CreditMessage.class);

        consumer.consume(message);

        verify(notificationService).notifyRejected(message);
    }
}