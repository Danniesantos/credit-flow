...
class CreditCanceledConsumerTest {

    @Mock
    private NotificationService notificationService;

    @InjectMocks
    private CreditCanceledConsumer consumer;

    @Test
    void shouldNotifyCanceledCredit() {
        CreditMessage message = mock(CreditMessage.class);

        consumer.consume(message);

        verify(notificationService).notifyCanceled(message);
    }
}