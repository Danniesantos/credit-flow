...
class CreditApprovedConsumerTest {

    @Mock
    private NotificationService notificationService;

    @InjectMocks
    private CreditApprovedConsumer consumer;

    @Test
    void shouldNotifyApprovedCredit() {
        CreditMessage message = mock(CreditMessage.class);

        consumer.consume(message);

        verify(notificationService).notifyApproved(message);
    }
}