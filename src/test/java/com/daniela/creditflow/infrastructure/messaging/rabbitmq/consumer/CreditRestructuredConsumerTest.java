...
class CreditRestructuredConsumerTest {

    @Mock
    private NotificationService notificationService;

    @InjectMocks
    private CreditRestructuredConsumer consumer;

    @Test
    void shouldNotifyRestructuredCredit() {
        CreditMessage message = mock(CreditMessage.class);

        consumer.consume(message);

        verify(notificationService).notifyRestructured(message);
    }
