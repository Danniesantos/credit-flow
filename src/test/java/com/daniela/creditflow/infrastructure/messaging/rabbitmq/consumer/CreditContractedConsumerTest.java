...
class CreditContractedConsumerTest {

    @Mock
    private NotificationService notificationService;

    @InjectMocks
    private CreditContractedConsumer consumer;

    @Test
    void shouldNotifyContractedCredit() {
        CreditMessage message = mock(CreditMessage.class);

        consumer.consume(message);

        verify(notificationService).notifyContracted(message);
    }
}